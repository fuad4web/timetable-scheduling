...
include('../dist/includes/dbcon.php');
	...
	foreach ($first as $daym){
		...
		if (($count_t==0) and ($count_r==0) and ($count_c==0))
		{
			mysqli_query($con,"INSERT INTO exam_sched(time_id,day,member_id,subject_code,cys,cys1,room,remarks,settings_id,term,encoded_by) 
				VALUES('$daym','first','$member','$subject','$cys','$cys1','$room','$remarks','$set_id','$term','$id')")or die(mysqli_error($con));
	...
	foreach ($second as $daym){
		...
		{
			mysqli_query($con,"INSERT INTO exam_sched(time_id,day,member_id,subject_code,cys,cys1,room,remarks,settings_id,term,encoded_by) 
				VALUES('$daym','second','$member','$subject','$cys','$cys1','$room','$remarks','$set_id','$term','$id')")or die(mysqli_error($con));
	...
	foreach ($third as $daym){
		...
		{
			mysqli_query($con,"INSERT INTO exam_sched(time_id,day,member_id,subject_code,cys,cys1,room,remarks,settings_id,term,encoded_by) 
				VALUES('$daym','third','$member','$subject','$cys','$cys1','$room','$remarks','$set_id','$term','$id')")or die(mysqli_error($con));
